...
- **One SDK, explicit external installation grants.** Release signatures establish origin and the in-process execution path; they are not the only way to grant SDK access. External packages stay in isolated Deno processes. Their immutable manifest and source digest require an authenticated `plugins.install` decision (Owner by default), with separate receipts for every requested capability. Declarations never grant access. The initial external brokers support `cms_admin_schema`, `chat_runner_tools`, declarative `companion_skills` authenticated `private_files` and author-only `image_generation`; all other elevated requests fail activation until their broker is implemented. Private author access also requires a host-authenticated author with `content.write`; visitor and rendering contexts receive the base SDK. Revocation invalidates exact receipts and serializes against storage commits. The legacy `tier` records the execution path, not a substitute for receipt checks.
- **Runtime-authored plugins never bypass the sandbox.** Deno subprocess + oxc-parser validator + schema declaration + Shadow DOM on every Web Component — disabling any one is a security regression, not an optimisation. The validator runs at activation time on runtime-authored plugins and as defense-in-depth on release-signed startup. There is NO unverified load path (#387): even in-memory test plugins run the full manifest-validator + signature pipeline.
- **AI authors runtime-authored plugins only.** AI cannot add a new file to `packages/plugins/<slug>/`. Release-signed source comes from human contributors auditing the change, signing the manifest, and shipping with a Caelo release. AI proposing new core capabilities means proposing a release-plugin PR text — it does not edit core plugin source itself.
- **Activation is a hard state, for EVERY provenance.** Discovery records a plugin; it does not start one. Until an Owner activates it at `/security/plugins`, nothing of the plugin is loaded — no schema, no tools, no skills, no workers, no URL or head contributions — and to the running system and the AI's tool catalogue it does not exist. This is not a display filter over a loaded plugin: an inactive plugin's tools are ABSENT from the catalogue, not filtered out of it. Release-signed provenance buys audit and a higher capability ceiling, never auto-start. The two deliberate exceptions, so the AI can notice a dormant capability and offer the one click instead of improvising around it: a short installed-but-inactive line in the system prompt, and the `list_plugins` tool. Activation loads the plugin into the running host immediately (`loadActivatedPlugin`, re-verifying the signature) — "activated but nothing happens until restart" is itself a bug.
- **A plugin's skills are part of the plugin.** They go live with it and archive with it; they do NOT need a second Owner click. Requiring one would leave the AI able to CALL a plugin's tools while the `## Skills` index — the only surface that announces a capability — stayed silent, which is how a shipped plugin ends up never used. The two-level model still holds for skills that are not plugin-owned: AI-authored proposals land `awaiting_activation` and wait for the Owner.
- **Skill activation never rewrites a running chat's system prompt.** The `## Skills` index sits in the cached prefix, so it is pinned to what was active when the chat began (`skills.activated_at`, migration 0213). A skill activated mid-chat is fully usable — its tools dispatch, its body loads — it is simply announced through a notice in the message history instead of by editing the cached prefix (CLAUDE.md §11).
...
- **Two levels of skill activation — never conflate them.** Site-wide activation promotes a skill from `awaiting_activation` to `active` — a human Owner click for a standalone skill, or the plugin's own activation for a plugin-owned one (see above). Per-chat engagement (AI auto-matcher or user manual toggle) decides which active skills augment the current AI call. User manual disengagement in a chat always overrides the matcher for that chat.
...
>
> **The propose/execute machinery below survives as the internal apply + audit engine.** A gated tool's SDK `execute`, after approval, chains the existing `<domain>.propose_<action>` (writes the pending row + preview + audit) then `<domain>.execute_proposal` (applies, Owner scope) — reusing every domain's correct apply logic (incl. multi-op fan-outs) with zero reimplementation. So the `<domain>_pending_actions` tables + the propose/execute ops are NOT retired; they are no longer an AI-facing choreography. The `/security/<domain>/pending` queue is vestigial for these 25 domains (rows are written+applied atomically, never left pending) but is retained for the bespoke gated tools that keep their own multi-step flows (`propose_skill`, `propose_site_import`, `create_layout`, `tune_rate_limit`, `site_memory_propose`).
>
> **To gate a NEW uniform domain:** author it with `makeProposeTool` — it now derives the executeOp, marks the tool gated, and the chat-runner wires the SDK approval automatically. Nothing else to do.
...
| plugins | activate | Per CMS_REQUIREMENTS §14.2 (restated in the §17.4 AI-restrictions list). Activating runs code that was not running: new tools, skills, workers, and URL contributions that reshape live paths. AI-proposable via `activate_plugin`; the plugin loads on the Owner's in-chat Approve, and its tools reach the AI on the NEXT turn (the approved turn's tool list was fixed before the card). |
...
| **Conversation history** | persist + replay `response.messages` | ✅ **closed (Option C, 2026-07)** — `chat_messages.response_messages` persists the SDK's canonical assembly per turn; `buildProviderHistory` replays it verbatim (`sdkMessages` passthrough), no rebuild from `fullStream` |
| **Provider-executed tools** (tool search, web search/fetch, code exec) | the SDK pairs `server_tool_use` + its result in `response.messages` | ✅ **closed (Option C)** — replayed inside the response-messages passthrough, correctly paired; no longer dropped |
| **Reasoning / thinking** | reasoning parts + signatures ride in `response.messages` | ✅ **closed (Option C)** — replayed via the passthrough; the hand-rolled `thinkingBlocks` + signature path survives only as the errored/aborted fallback |
| **Structured output** | `generateObject({ schema })` (no-tools case) / `Output.object` on streamText (tools-live case) | ✅ **closed (Plan A, 2026-07)** — moduleize uses `provider.generateObject` (SDK `generateObject`); the forced `submit_*`-tool path + `toolChoice` are gone. (Subagent `submit_result` stays a normal agentic tool — tools must stay live, not a structured-output call.) |
| Tool *definition* / dispatch loop | `tool({ inputSchema, execute })` + the SDK's tool loop | ✅ **closed (issue #442, 2026-08)** — the SDK's multi-step loop IS the chat loop: every tool ships an `execute` (routine tools wrap the full dispatch pipeline; gated tools the propose→execute chain), and snapshot emission, audit, cost cap, compaction, and subagent spawning ride `stopWhen`/`prepareStep`/`onStepFinish` + the execute wrappers. Provider-deferred tool results (tool search) are consumed in-turn, which removed the pairing-400 wedge class. |
| **Tool approval** (human-in-the-loop) | `toolApproval` / `needsApproval` (v7) | ✅ **closed (Plan B, 2026-07)** — gated tools ship `approvalMode:"user-approval"` + an `execute`; the SDK pauses on `tool-approval-request`, the operator approves IN CHAT, resume appends the SDK `tool-approval-response`. The propose/execute machinery survives as the internal apply+audit engine (§11.A). |
...
| Agent loop | `Agent` / `ToolLoopAgent` + `stopWhen` | ✅ **closed for the loop, by design for the class (issue #442)** — we run `streamText`'s multi-step loop directly, NOT the Agent/ToolLoopAgent class: the class adds nothing over the hooks we already use, while the chat-runner's outer wrapper still owns cross-run policy (one-shot retries, approval resume, notices) that has no class equivalent. |
