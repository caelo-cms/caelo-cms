...
- **One SDK, provenance is the trust ceiling (epic #380 decision 2).** Every plugin imports the same `@caelo-cms/plugin-sdk`; every declared capability is runtime-enforced. What is *grantable* is capped by **provenance**: `release-signed` plugins (`packages/plugins/<slug>/`, audited, Ed25519-verified manifest, run in-process) may request any capability — cross-`cms_admin` writes, snapshot emission, chat-runner tools, AI provider access, background workers. `runtime-authored` plugins (AI-authored or Owner-installed at runtime, source in `plugins.source_code`) get the sandbox base (`ctx.query`/`api`/`theme`/`visitor`/`captcha`) and nothing else — the validator rejects a runtime-authored manifest reaching over the ceiling, and the capability factory independently refuses at runtime. The legacy `tier` column (1|2) persists the same fact as derived trust — never treat it as a parallel interface.
