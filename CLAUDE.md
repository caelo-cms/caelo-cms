...
- **Release-signed activation = verified manifest + auto-on-install.** Owner can disable from `/security/plugins` but is not asked to click Approve on first run for code shipped with Caelo. **Runtime-authored activation = Owner click per `active` transition.** AI submits; human Owner flips to active. No auto-activation, ever.
