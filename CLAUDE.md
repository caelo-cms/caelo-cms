...
| Tool *definition* / dispatch loop | `tool({ inputSchema, execute })` + the SDK's tool loop | ✅ **by design** — routine tools pass schemas but NO `execute`; the chat-runner owns the loop so snapshot emission, audit, cost cap, and subagent spawning happen between calls. Keep. |
