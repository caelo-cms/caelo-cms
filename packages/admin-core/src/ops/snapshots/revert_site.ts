...
import { err, ok, revertSiteInput } from "@caelo-cms/shared";
...
export const revertSiteOp = defineOperation({
  ...
  handler: async (ctx, input, tx) => {
    ...
    for (const r of parsedLayouts) {
      ...
      // Prefer the snapshot's `placements` array (v0.12+ writers always
      // populate it); fall back to minting fresh unsynced content_instances
      // for pre-v0.12 snapshots whose state only carries `moduleIds`.
      ...
      for (const block of t.blocks) {
        ...
        if (placements.length > 0) {
          ...
          for (const moduleId of block.moduleIds) {
            ...
            if (!newCiId) {
              throw new Error(
                `revert_site: failed to mint content_instance for legacy pageLayout snapshot (page=${r.entity_id} block=${block.blockName} pos=${position})`,
              );
            }
            await tx.execute(sql`
              INSERT INTO page_modules
