    ...
    if (pageR.ok) {
      const v = pageR.value as {
        page: {
          ...
          status: string;
          templateId: string;
          /** 0184 — this page is the designated site root. */
          isHomePage: boolean;
          blocks: {
            blockName: string;
      ...
      const lines: string[] = [
        "# Current page",
        `Page: ${v.page.slug} (status=${v.page.status}, id=${v.page.id})`,
        ...(v.page.isHomePage
          ? [
              "Homepage: YES — this page is the site root and serves at `/` (not at `/<slug>`). It keeps its slug but the designation (set_home_page) makes it the site root.",
            ]
          : []),
      ...
      lines.push(
        ...
        "- set_structured_set(kind, slug, displayName, items) — upsert a structured-data set (nav-menu, tags, taxonomy, link-list). Pass the FULL desired item list — op REPLACES, not appends. For partial updates (one theme token, one link rename), call `get_structured_set` first to read current items, mutate in JS, then `set_structured_set` with the merged array. The system-prompt block above already inlines nav-menu items (up to 30/menu) at session start; copy them and modify, don't re-invent.",
        "",
