...
export const screenshotPageTool: ToolDefinitionWithHandler<ScreenshotPageInput> = {
  ...
  description:
    "Capture a screenshot of the rendered page (operator's browser does the capture via html2canvas; the image comes back WITHIN this same turn — keep working and analyse it on your next step, do NOT end your turn to wait for it). Use for the GROSS visual impression — 'does the hero feel crowded?', 'is the color/imagery right?', 'what's the overall layout impression?'. " +
    "FIDELITY LIMIT — html2canvas is an APPROXIMATION, not a real browser render. It reliably mis-renders TWO things: (1) fine text spacing — words can show large irregular gaps that look like broken text-align:justify, and (2) flex-wrap — wrapping rows can stack one-item-per-line. These are html2canvas artifacts, NOT real CSS bugs. If a screenshot shows justified-looking word gaps or single-column flex stacking, DO NOT edit CSS or file a bug from the screenshot alone — confirm against `inspect_page_render` first, and treat its composed CSS as the source of truth for spacing + layout. " +
    ...
    "For VERIFYING A REBUILT IMPORTED PAGE against its original prefer `verify_import_page_fidelity` — it diffs the source screenshot against your rebuild and returns pass/warn/fail numbers synchronously, no browser needed. For ANY spacing/layout/CSS question (word gaps, flex stacking, white halo around the header, wrong colors, broken layout) prefer `inspect_page_render` — it returns the HTML + every CSS layer separately, is the accurate source of truth, and is faster + cheaper. " +
