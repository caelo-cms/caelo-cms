...
export const checkPageContentInventoryTool: ToolDefinitionWithHandler<InventoryInput> = {
  ...
  description:
    "Verify a rebuilt imported page lost NO content from the source. Call it right AFTER you rebuild a page's modules with `build_page`. It compares the source page's crawled content against the rebuilt modules and returns what is covered plus a LOUD list of every missing heading, paragraph, list item, image, link, or CTA (with the source context so you can find it). Content is sacred: restore anything genuinely dropped, or record WHY you dropped it (add_import_page_notes) so the operator sees it — never let content vanish silently. Chrome (header/footer) is excluded by default because it is layout-owned. `importPageId` accepts the staging import_pages id (from list_import_pages / get_import_page — this ALWAYS resolves), the composed page id, or the built CMS page id (matched by the build_page importPageId link, by slug, or — for the homepage built at a custom slug — by its locale home-page designation). If a built page can't be found (you gave it a custom/translated slug), pass the staging import_pages id instead. Import data does not expire, so a miss is an id/slug mismatch, not staleness.",
