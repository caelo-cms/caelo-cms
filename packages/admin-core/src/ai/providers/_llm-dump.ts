...
/**
 * Assembly-time capture of the per-turn prefix — fires regardless of the
 * provider (so a FixtureProvider run captures the REAL system + tools +
 * history WITHOUT a billed API call). Records per-system-chunk sizes (with
 * cacheable flag), per-tool sizes, and per-history-message sizes. No-op
 * when disarmed.
 */
export function writeAssemblyDump(args: {
  chatSessionId: string;
  systemChunks: readonly { body: string; cacheable: boolean; label: string }[];
  ...
  if (!p) return;
  try {
    const systemChunks = args.systemChunks.map((c) => ({
      label: c.label,
      cacheable: c.cacheable,
      chars: c.body.length,
      estTokens: estTokens(c.body.length),
    }));
    const tools = args.tools.map((t) => {
      const chars = JSON.stringify(t).length;
      return { name: t.name, chars, estTokens: estTokens(chars) };
    });
    const messages = args.messages.map((m, i) => {
      const chars = JSON.stringify(m.content ?? "").length;
      return { label: `${m.role}[${i}]`, chars, estTokens: estTokens(chars) };
    });
    const systemChars = systemChunks.reduce((a, s) => a + s.chars, 0);
    const toolChars = tools.reduce((a, s) => a + s.chars, 0);
    const messageChars = messages.reduce((a, s) => a + s.chars, 0);
    const rec = {
      ...
      totals: {
        systemChars,
        systemEstTokens: estTokens(systemChars),
        toolChars,
        toolEstTokens: estTokens(toolChars),
        messageChars,
        messageEstTokens: estTokens(messageChars),
        totalEstTokens: estTokens(systemChars + toolChars + messageChars),
      },
    };
    appendFileSync(p, `${JSON.stringify(rec)}\n`);
  } catch {
