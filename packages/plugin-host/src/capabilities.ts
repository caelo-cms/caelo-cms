...
export async function makePluginContext(
  ...
  if (plugin.provenance === "runtime-authored") {
    ...
    if (plugin.externalApproval.capabilities.includes("font_assets"))
      extended.fonts = makePluginFonts(plugin, infra, author);
    return extended;
  }
  ...
  if (requested.has("font_assets") && opts.authorContext && !visitorContext)
    tier1.fonts = makePluginFonts(plugin, infra, opts.authorContext);
  return tier1;
}
