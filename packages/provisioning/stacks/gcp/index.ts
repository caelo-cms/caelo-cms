...
// v0.2.81 — secrets MUST be generated by a Pulumi resource that
// persists in stack state, NOT by `randomHex(32)` at program-eval
// time. Pulumi re-evaluates the program on every `pulumi up`, so
// any plain-JS random() call produces a new value each run, and
// the matching SecretVersion gets re-created with the new value.
// Symptom: after a re-up, all secrets encrypted under the old KEK
// (AI provider API keys in ai_providers) became permanently
// unreadable — and Secret Manager destroyed the prior versions
// after their soft-delete window so recovery was impossible.
//
// `random.RandomBytes` materialises ONCE, persists the bytes in
// state, reuses on subsequent runs. This is the canonical fix for
// "auto-generated but stable" secret values in Pulumi.
// random.RandomBytes uses the @pulumi/random provider, not the gcp
// one — don't pass `opts` (which carries gcpProvider). Pulumi
// auto-loads the random provider on first reference.
...
const imageTagSource = cfg.get("image-tag") ?? "latest";
...
function imageTag(service: string): pulumi.Output<string> {
  ...
  // (resolves :latest → digest via the AR tags list) so each pulumi up
  // rolls Cloud Run to the newest release image even though the
  // `:latest` tag is mutable. Without this Cloud Run would silently
  // keep running an old image after a release.
