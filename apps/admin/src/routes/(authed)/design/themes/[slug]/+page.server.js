...
export const actions = {
    ...
    removeToken: async ({ request, params, locals }) => {
        ...
        return {
            ok: true,
            message: v.canonicalPathsRemoved.length > 0
    ...
    setAsset: async ({ request, params, locals }) => {
        ...
        const validSlots = ["logo", "logoDark", "favicon", "socialShare"];
        if (!validSlots.includes(slotRaw)) {
 ...
 */
function coerceFormValue(name, raw) {
