...
export const load = async ({ locals }) => {
    ...
    const themes = themesR.ok ? themesR.value.themes : [];
    const pendingCount = pendingR.ok
        ? pendingR.value.proposals.length
        : 0;
...
export const actions = {
    ...
    create: async ({ request, locals }) => {
        ...
        if (primaryColor.length > 0)
            overrides.primaryColor = primaryColor;
        const { adapter, registry } = getQueryContext();
        ...
        const active = activeR.value.theme;
        if (!active) {
            return fail(400, {
                error: "no active theme to base the new variant on — activate a theme first (or let the AI compose one in chat).",
            });
        }
