 ...
 *
 * GET `/edit/preview-by-path/<...slug>?branch=<chatBranchId>`
 * resolves the slug to a pageId via `pages.list`, then delegates to
 ...
 * `/edit/preview-by-path/about` — no JS interception required for
...
export const GET: RequestHandler = async ({ params, url, locals }) => {
  ...
  // Resolve the slug via `pages.list`. The path can contain slashes
  // (e.g. `blog/hello-world`) — multi-segment slugs are honoured
  // verbatim because `pages.slug` is already free-form text.
  ...
  if (!page) throw error(404, `No page at ${slug}`);
  ...
  const ctx = `window.__caelo=${JSON.stringify({ pageId: page.id, slug })};`;
