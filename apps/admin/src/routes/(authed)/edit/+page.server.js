...
import { DEFAULT_LAYOUT, } from "$lib/components/edit/use-overlay-layout.svelte.js";
...
export const load = async ({ locals, url }) => {
    ...
    if (!editUser.permissions.has("content.write"))
        throw redirect(303, "/");
    ...
    // on the chat's branch (build_page, ...).
    let pages = pagesR.ok ? pagesR.value.pages : [];
    ...
    let activePage = pages.find((p) => p.id === queryPage) ??
    ...
    const sessions = sessionsR.ok ? sessionsR.value.sessions : [];
    const globalSessions = globalSessionsR.ok
        ? globalSessionsR.value.sessions
        : [];
    ...
    let activeChat = sessions.find((s) => s.id === queryChat) ??
    ...
    if (!activeChat) {
        ...
        throw error(500, "Could not start a live-edit session — the chat ops returned no usable row. Check Cloud Run logs for `[edit]` stderr lines, or visit /security/audit.");
    }
    ...
    if (branchPagesR.ok) {
        pages = branchPagesR.value.pages;
        activePage =
    ...
    if (activeChat && pages.length === 0) {
        ...
        if (untouched && messages.length === 0) {
            ...
            if (seeded.ok) {
                ...
                if (reread.ok) {
                    messages = reread.value.messages;
                }
            }
            else {
    ...
    if (activeChat) {
        ...
        if (editedR.ok) {
            branchEditedModuleIds = editedR.value.moduleIds;
        }
        const countR = await execute(registry, adapter, locals.ctx, "chat.branch_change_count", {
            chatSessionId: activeChat.id,
        });
        if (countR.ok) {
            branchChangeCount = countR.value.count;
        }
    }
    let otherOpenChats = [];
    if (activeChat) {
        ...
        if (othersR.ok) {
            otherOpenChats = othersR.value.chats;
        }
    }
    ...
    let lastPublishedLive = null;
    let productionMatchesStaging = null;
    try {
        const recentR = await execute(registry, adapter, locals.ctx, "deploy.list_runs", {
            limit: 20,
        });
        if (recentR.ok) {
            const v = recentR.value;
            const r = v.runs.find((row) => row.targetName === "staging" && row.status === "succeeded" && row.finishedAt);
            if (r?.finishedAt) {
                lastStaged = {
                    runId: r.id,
            ...
            const p = v.runs.find((row) => row.targetName === "production" && row.status === "succeeded" && row.finishedAt);
            if (p?.finishedAt) {
                lastPublishedLive = {
                    runId: p.id,
    ...
    if (activeChat) {
        ...
        if (pR.ok)
            pendingChanges = pR.value;
    }
    // Layout preference — default if unset.
    let layout = DEFAULT_LAYOUT;
    if (prefsR.ok) {
        const v = prefsR.value.value;
        if (v && typeof v === "object") {
            layout = { ...DEFAULT_LAYOUT, ...v };
        }
    }
    ...
    if (activePage && activePageId) {
        ...
        if (fullR.ok) {
            ...
            if (p && p.translationStatus !== "source") {
                ...
                if (sourceR.ok) {
                    const list = sourceR.value.pages;
...
export const actions = {
    ...
    newChat: async ({ request, locals, url }) => {
        ...
        const newId = created.value.chatSessionId;
        const next = new URL(url);
    ...
    setPageStatus: async ({ request, locals }) => {
        ...
        if (!pageId)
            return fail(400, { error: "missing pageId" });
        if (status !== "draft" && status !== "published") {
            return fail(400, { error: "status must be 'draft' or 'published'" });
        }
        const ctx = chatBranchId ? { ...locals.ctx, chatBranchId } : locals.ctx;
    ...
    stageAndDeployStaging: async ({ request, locals }) => {
        ...
        if (!chatSessionId)
            return fail(400, { error: "missing chatSessionId" });
        const pageId = String(form.get("pageId") ?? "");
        ...
        const mergedSummaryValue = merged.value;
        ...
        const summary = stagingDeploy.value;
        let previewUrl;
        if (summary.previewUrl) {
            ...
            if (pageRow.ok && localesR.ok) {
                const p = pageRow.value.page;
                const locales = localesR.value.locales;
        ...
        if (pagesForDraftCount.ok) {
            const allPages = pagesForDraftCount.value.pages;
    ...
    publishToProduction: async ({ request, locals }) => {
        ...
        const checked = new Set(rawKinds.filter((k) => allowed.includes(k)));
        let changedPageIds;
        ...
        const fullRebuild = checked.size === 0 ||
            checked.size === allowed.length ||
            checked.has("lists") ||
            checked.has("layoutChrome");
        if (!fullRebuild && chatSessionId) {
            const entitiesR = await execute(registry, adapter, locals.ctx, "chat.branch_edited_entities", { chatSessionId });
            if (entitiesR.ok) {
                const entities = entitiesR.value;
                const impactR = await execute(registry, adapter, locals.ctx, "snapshots.publish_impact_pages", {
                ...
                if (impactR.ok) {
                    ...
                    if (impact.fullSite) {
                        ...
                        for (const id of impact.pageIds)
                            all.add(id);
                    }
                }
                if (checked.has("pages")) {
                    for (const id of entities.pageIds)
                        all.add(id);
                    for (const id of entities.pageLayoutPageIds)
                        all.add(id);
                }
        ...
        const summary = productionDeploy.value;
        return {
            published: {
                targetName: "production",
                pageCount: summary.pageCount,
