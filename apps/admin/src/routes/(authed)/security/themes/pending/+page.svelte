...
<div class="space-y-6">
  <div>
    <h1 class="text-2xl font-semibold tracking-tight">Pending theme actions</h1>
    <p class="text-sm text-muted-foreground">
      AI-proposed theme create / activate / delete actions wait here for your click. The preview
      shows the realised tokens (for create) or the active-theme swap (for activate). Note: approving
      an <em>activate</em> flips the DB row only — the live site keeps serving the previously-active
      theme's CSS until you separately approve a deploy at <code>/security/deployments/pending</code>.
    </p>
  </div>
  ...
  {#if data.proposals.length === 0}
    <Card>
      <CardContent class="py-12 text-center text-sm text-muted-foreground">
        No pending theme proposals. AI-driven theme requests will land here.
      </CardContent>
    </Card>
  {:else}
    <div class="space-y-4">
