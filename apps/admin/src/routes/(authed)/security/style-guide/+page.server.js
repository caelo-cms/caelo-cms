...
export const load = async ({ locals }) => {
    ...
    const guides = guidesR.ok
        ? guidesR.value.guides
        : [];
    const locales = localesR.ok
        ? localesR.value.locales
        : [];
...
export const actions = {
    upsert: async ({ request, locals }) => {
        ...
        if (!locale || !body)
            return fail(400, { error: "locale and body required" });
        if (body.length > 4000)
            return fail(400, { error: "body must be ≤ 4000 chars" });
        const { adapter, registry } = getQueryContext();
    ...
    delete: async ({ request, locals }) => {
        ...
        if (!r.ok)
            return fail(400, { error: "delete failed" });
