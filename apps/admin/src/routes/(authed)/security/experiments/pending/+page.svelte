...
<div class="space-y-6">
  <div>
    <h1 class="text-2xl font-semibold tracking-tight">Pending experiment actions</h1>
    <p class="text-sm text-muted-foreground">
      AI-proposed experiment activations + completions wait here for your click. The preview shows
      the experiment's variant split (for activate) or current per-variant assignment counts (for
      complete) so you can sanity-check the AI's proposed winner before recording it.
    </p>
  </div>
  ...
  {#if data.proposals.length === 0}
    <Card>
      <CardContent class="py-12 text-center text-sm text-muted-foreground">
        No pending experiment proposals. AI-driven activate/complete requests will land here.
      </CardContent>
    </Card>
  {:else}
    <div class="space-y-4">
