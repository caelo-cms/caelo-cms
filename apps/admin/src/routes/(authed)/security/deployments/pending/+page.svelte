...
<div class="space-y-6">
  <div>
    <h1 class="text-2xl font-semibold tracking-tight">Pending deploy actions</h1>
    <p class="text-sm text-muted-foreground">
      AI-proposed promote / rollback actions wait here for your click. Approving runs the underlying
      op as if you'd clicked the deploy button yourself; rejecting marks the proposal as declined.
      No deploys move without an explicit Approve.
    </p>
  </div>
  ...
  {#if data.proposals.length === 0}
    <Card>
      <CardContent class="py-12 text-center text-sm text-muted-foreground">
        No pending deploy proposals. AI-driven promote / rollback requests will land here.
      </CardContent>
    </Card>
  {:else}
    <div class="space-y-4">
