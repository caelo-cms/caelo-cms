...
export const actions = {
    approve: async ({ request, locals }) => {
        ...
        if (!claim.ok) {
            const message = typeof claim.error === "object" && claim.error && "message" in claim.error
                ? String(claim.error.message)
                : "approve failed";
            return fail(400, { error: message });
        }
        const { toolName, args, chatSessionId } = claim.value;
        ...
        if (chatSessionId) {
            try {
                await execute(registry, adapter, locals.ctx, "chat.append_message", {
                    chatSessionId,
                    role: "user",
                    content: `[Owner approved proposal ${proposalId} and the tool was dispatched] ${toolName}: ${dispatchResult.content}`.slice(0, 8000),
