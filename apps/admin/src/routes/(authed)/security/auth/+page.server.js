...
export const load = async ({ locals }) => {
    ...
    if (!r.ok) {
        return {
            config: { id: null, signupOpen: true, passwordMinLength: 8, updatedAt: null },
            error: r.error.message,
        };
    }
    return { config: r.value, error: null };
};
export const actions = {
    apply: async ({ request, locals }) => {
        ...
        const passwordMinLength = Number.parseInt(typeof passwordMinLengthRaw === "string" ? passwordMinLengthRaw : "8", 10);
