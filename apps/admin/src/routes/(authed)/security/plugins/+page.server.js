...
export const load = async ({ locals }) => {
    ...
    const plugins = r.ok ? r.value.plugins : [];
    const tier1 = plugins.filter((p) => p.tier === 1);
    const tier2Active = plugins.filter((p) => p.tier === 2 && p.status === "active");
    const tier2AwaitingActivation = plugins.filter((p) => p.tier === 2 && p.status === "awaiting_activation");
    const tier2Failed = plugins.filter((p) => p.tier === 2 && (p.status === "draft" || p.status === "failed"));
...
export const actions = {
    activate: async ({ request, locals }) => {
        ...
        if (typeof slug !== "string" || !slug)
            return fail(400, { error: "slug required" });
        const { adapter, registry } = getQueryContext();
        ...
        if (!prepared.isReEnable) {
            try {
                ...
                return fail(500, {
                    error: `cms_public provisioning failed: ${e.message}`,
                });
            }
        }
        ...
        if (!commit.ok) {
            if (!prepared.isReEnable) {
                try {
                    ...
                    return fail(500, {
                        error: `commit failed (${commit.error.kind}); ALSO rollback failed: ${rollbackError.message}. Manual cleanup needed for cms_public schema "${prepared.schemaName}".`,
                    });
                }
            }
