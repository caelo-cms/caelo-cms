...
export const load = async ({ locals }) => {
    ...
    const roles = rolesResult.ok
        ? rolesResult.value.roles.map((r) => r.name)
        : [];
    return { users, roles };
};
export const actions = {
    ...
    setRoles: async ({ request, locals }) => {
        ...
        if (!result.ok)
            return fail(400, { error: "Could not update roles." });
        return { ok: true };
    },
    delete: async ({ request, locals }) => {
        ...
        if (!result.ok)
            return fail(400, { error: "Could not delete user." });
        return { ok: true };
    },
    resetPassword: async ({ request, locals }) => {
        ...
        const result = await execute(registry, adapter, { ...locals.ctx, actorKind: "system" }, "users.admin_set_password", { userId, newPassword });
        if (!result.ok) {
