 ...
 */
import { buildEmailTransport } from "@caelo-cms/admin-core";
...
export const actions = {
    ...
    testSend: async ({ request, locals }) => {
        ...
        const r = await execute(registry, adapter, locals.ctx, "email_config.get", {});
        if (!r.ok)
            return fail(500, { error: `Could not load email config: ${r.error.kind}` });
        ...
        try {
            ...
            return fail(500, { error: `Send failed: ${e.message}` });
        }
    },
};
