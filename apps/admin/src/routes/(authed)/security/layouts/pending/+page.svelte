...
<div class="space-y-6">
  <div>
    <h1 class="text-2xl font-semibold tracking-tight">Pending layout actions</h1>
    <p class="text-sm text-muted-foreground">
      AI-proposed layout create / update / delete / set_blocks actions wait here for your click.
      The preview shows the affected templates + pages so you can see the blast radius before
      approving. No layouts change without an explicit Approve.
    </p>
  </div>
  ...
  {#if data.proposals.length === 0}
    <Card>
      <CardContent class="py-12 text-center text-sm text-muted-foreground">
        No pending layout proposals. AI-driven layout requests will land here.
      </CardContent>
    </Card>
  {:else}
    <div class="space-y-4">
