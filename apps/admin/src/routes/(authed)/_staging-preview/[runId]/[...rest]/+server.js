...
async function getBuckets() {
    ...
    if (!stagingBucketName) {
        throw new Error("_staging-preview: CAELO_STAGING_BUCKET not set. Cloud Run must set this env var (provisioner adds it for GCP installs).");
    }
    if (!staticBucketName) {
        throw new Error("_staging-preview: CAELO_STATIC_BUCKET not set. Cloud Run must set this env var.");
    }
...
export const GET = async ({ params }) => {
    ...
    if (!runId)
        throw error(400, "runId required");
    ...
    if (provider !== "gcp") {
        throw error(404, `_staging-preview only available on GCP installs (CAELO_PROVIDER='${provider ?? "self-hosted"}'). Self-hosted installs view staged builds at staging.<domain>.`);
    }
