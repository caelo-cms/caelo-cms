...
export const load = async ({ params, locals }) => {
    ...
    return { template: result.value.template };
};
function parseBlocks(form) {
    ...
    while (form.has(`blockName.${i}`)) {
        ...
        if (name.length > 0)
            out.push({ name, displayName: displayName || name, position: i });
        i += 1;
    }
    return out;
}
export const actions = {
    update: async ({ params, request, locals }) => {
        ...
        if (!result.ok)
            return fail(400, { error: "Could not update template." });
        return { ok: true };
    },
    setBlocks: async ({ params, request, locals }) => {
        ...
        if (!result.ok)
            return fail(400, { error: "Could not save blocks." });
        return { ok: true };
    },
    delete: async ({ params, request, locals }) => {
        ...
        if (!result.ok)
            return fail(400, { error: "Could not delete template (still in use?)." });
