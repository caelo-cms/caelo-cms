...
export const load = async ({ locals }) => {
    ...
    const matrixR = await execute(registry, adapter, locals.ctx, "pages.translation_status_matrix", {});
    const jobsR = await execute(registry, adapter, locals.ctx, "translation_jobs.list", {
        status: "any",
        limit: 50,
    });
    const matrix = matrixR.ok ? matrixR.value.rows : [];
    const jobs = jobsR.ok ? jobsR.value.jobs : [];
    ...
    const localesUnique = [...new Map(localesAll.map((l) => [l.code, l])).values()].sort((a, b) => a.code.localeCompare(b.code));
    const cellsByKey = {};
    for (const r of matrix)
        cellsByKey[`${r.slug}|${r.locale}`] = r;
    // Total stale-ish count for the bulk strip.
    const staleCount = matrix.filter((r) => r.status === "not_started" || r.status === "needs_update").length;
...
export const actions = {
    bulkAllStale: async ({ request, locals }) => {
        ...
        if (!r.ok) {
            const m = typeof r.error === "object" && r.error && "message" in r.error
    ...
    cancelJob: async ({ request, locals }) => {
        ...
        if (!r.ok)
            return fail(400, { error: "cancel failed" });
