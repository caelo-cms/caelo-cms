...
export const actions = {
    bulkLocale: async ({ request, locals }) => {
        ...
        if (!code)
            return fail(400, { error: "locale code required" });
        const { adapter, registry } = getQueryContext();
    ...
    updateCap: async ({ request, locals }) => {
        ...
        if (!r.ok)
            return fail(400, { error: "update_cap failed" });
    ...
    revertJob: async ({ request, locals }) => {
        ...
        if (!r.ok)
            return fail(400, { error: "revert failed" });
        const v = r.value;
        const parts = [
            `Reverted ${v.deletedVariants} new translation${v.deletedVariants === 1 ? "" : "s"}.`,
        ];
        if (v.needManualRevert > 0) {
            parts.push(`${v.needManualRevert} updated translation${v.needManualRevert === 1 ? "" : "s"} need per-page revert via the Advanced History drawer.`);
        }
    ...
    resetTranslationState: async ({ request, locals }) => {
        ...
        if (!variantPageId)
            return fail(400, { error: "variantPageId required" });
        const { adapter, registry } = getQueryContext();
        ...
        if (!r.ok)
            return fail(400, { error: "reset failed" });
        return {
            ok: true,
