...
export const load = async ({ params, locals }) => {
    ...
    const { instance, placements } = r.value;
...
export const actions = {
    save: async ({ request, params, locals }) => {
        ...
        if (!r.ok) {
            const msg = r.error.message ?? "could not save";
            return fail(400, { error: msg });
        }
        const { placementCount } = r.value;
        throw redirect(303, `/content/library?saved=${encodeURIComponent(params.id)}&placements=${placementCount}`);
    },
};
