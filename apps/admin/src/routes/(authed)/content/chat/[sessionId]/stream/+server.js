...
export const POST = async ({ params, request, locals }) => {
    ...
    const origin = body.origin === "system" ? "system" : undefined;
    ...
    const resumeApproval = body.resumeApproval &&
    ...
    if (!resumeApproval && (typeof body.content !== "string" || body.content.trim().length === 0)) {
        throw error(400, `chat message content must be a non-empty string (got ${typeof body.content === "string" ? "empty/whitespace" : typeof body.content}${origin ? ", origin=system" : ""}) — a status/nudge producer with nothing to say must not post a message at all`);
    }
        ...
        async start(controller) {
            ...
            try {
                ...
                const clientPayload = isAbort
                    ...
                    : { kind: "error", message: `Server error: ${message}` };
                try {
