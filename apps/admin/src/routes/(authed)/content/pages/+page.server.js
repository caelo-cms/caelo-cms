...
export const actions = {
    create: async ({ request, locals }) => {
        ...
        if (!result.ok)
            return fail(400, { error: "Could not create page." });
        const pageId = result.value.pageId;
    ...
    stage: async ({ request, locals }) => {
        ...
        if (!updateResult.ok)
            return fail(400, { error: "Could not mark page as published." });
    ...
    publishAllDrafts: async ({ request, locals }) => {
        ...
        const draftIds = listResult.value.pages
        ...
        for (let i = 0; i < draftIds.length; i += 200) {
            ...
            if (!r.ok) {
                ...
                return fail(500, {
                    error: `Published ${published} page(s), then a batch failed: ${describeError(r.error)}. ` +
                        "Re-run to publish the rest.",
                });
            }
            published += r.value.updatedCount;
        }
        return {
