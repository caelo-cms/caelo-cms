...
export const actions = {
    update: async ({ params, request, locals }) => {
        ...
        const expectedVersion = expectedVersionRaw === null ? undefined : Number.parseInt(String(expectedVersionRaw), 10);
    ...
    delete: async ({ params, request, locals }) => {
        ...
        if (!result.ok)
            return fail(400, { error: "Could not delete page." });
    ...
    setPlacementContent: async ({ params, request, locals }) => {
        ...
        if (!result.ok) {
            const message = result.error.message ?? "could not bind placement";
    ...
    forkPlacementContent: async ({ params, request, locals }) => {
        ...
        if (!result.ok) {
            const message = result.error.message ?? "could not fork placement";
            return fail(400, { error: message });
        }
        return { ok: true };
    },
