...
<div class="space-y-6">
  <div>
    <h1 class="text-2xl font-semibold tracking-tight">
      History — {data.page?.slug ?? data.pageId.slice(0, 8)}
    </h1>
    <p class="text-sm text-muted-foreground">
      Every snapshot that touched this page, reverse-chronological.
    </p>
  </div>
  ...
  {#if data.snapshots.length === 0}
    <Card>
      <CardContent class="py-8 text-center text-sm text-muted-foreground">
        <em>No snapshots for this page yet.</em>
      </CardContent>
    </Card>
  {:else}
    <ul class="space-y-2">
          ...
          <Card>
            <CardContent class="flex items-center justify-between gap-4 py-4">
              ...
              >
                ...
                <Button type="submit" variant="outline" size="sm">Revert this page to here</Button>
              </form>
            </CardContent>
          </Card>
        </li>
