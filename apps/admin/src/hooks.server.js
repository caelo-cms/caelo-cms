...
import { buildEmailTransport, configureMcpBridge, configureProviderResolver, emitSnapshot, generateKekHex, getActiveProvider, getMediaStorage, resetStuckTranslationUnits, setMode2Provider, setTranslationProvider, startChatImageGcWorker, startProposalGcWorker, startReleaseCheckWorker, startTranslationWorker, } from "@caelo-cms/admin-core";
...
import { bootstrap as bootstrapPluginHost, } from "@caelo-cms/plugin-host";
...
async function bootstrapTranslationWorker() {
    if (translationBootstrapped)
        return;
...
function makePluginHostAiProvider() {
    return {
        complete: async (opts) => {
...
async function bootstrapPlugins() {
    ...
    const aiProvider = makePluginHostAiProvider();
    ...
    if (!emailTransport) {
        ...
        if (emailUsers.length > 0) {
            console.warn(`[hooks] email transport NOT configured — ${emailUsers.map((p) => p.slug).join(", ")} will log sends to stderr only. Configure at /security/email.`);
        }
    }
...
async function consumePendingBootstrapToken() {
    if (pendingTokenInserted)
        return;
...
function bootstrapRedeploy() {
    if (redeployBootstrapped)
        return;
...
function bootstrapReleaseCheck() {
    if (releaseCheckBootstrapped)
        return;
...
function bootstrapProposalGc() {
    if (proposalGcBootstrapped)
        return;
...
function bootstrapChatImageGc() {
    if (chatImageGcBootstrapped)
        return;
...
function bootstrapMcpBridge() {
    if (mcpBridgeBootstrapped)
        return;
...
function ensureProviderResolverConfigured() {
...
export const handle = async ({ event, resolve }) => {
    ...
    if (token) {
        const requestCtx = { ...SYSTEM_CTX, requestId };
