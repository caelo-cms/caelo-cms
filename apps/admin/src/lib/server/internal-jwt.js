...
function getSecret() {
    ...
    if (!s || s.length < 32) {
        throw new Error(`${SECRET_ENV} not set or too short (need ≥32 chars). Required for /api/internal/*. ` +
 ...
 */
export async function signInternalToken(payload) {
 ...
 */
export async function verifyInternalToken(token, expectedScope) {
    let secret;
    try {
        secret = getSecret();
    }
 ...
 */
export async function requireInternalAuth(request, expectedScope) {
