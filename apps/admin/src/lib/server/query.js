...
import { getMediaStorageFactory, LocalVolumeAdapter, PostgresRateLimiter, registerAdminOps, setDeployBridge, setMediaStorage, } from "@caelo-cms/admin-core";
...
export function getQueryContext() {
    ...
    if (providerName === "local") {
        ...
        if (!factory) {
            throw new Error(`MEDIA_STORAGE_PROVIDER=${providerName} but no factory registered. ` +
...
export function verifyQueryContextRoles() {
    if (slot.verifyPromise)
