...
const NESTED_PROMPT = "On the homepage's hero block, create a 'cta-teaser' module that contains a 'button' module nested inside it via the `module` field kind. " +
...
function renderHomepagePreview() {
    ...
    const { pageId } = JSON.parse(raw.stdout);
    if (!pageId)
        return { html: "", missingSlots: ["no-homepage"] };
...
test("AC #2 + #5: CTA-teaser renders its embedded Button via the {{>cta}} slot", async ({ page, }) => {
