...
function seedThreePagesSynced() {
    ...
    if (raw.status !== 0)
        throw new Error(`seed failed: ${raw.stderr}`);
    return JSON.parse(raw.stdout);
}
function placementContentInstanceId(pageId, blockName, position) {
    const raw = spawnSync("bun", [
        "-e",
        `
        import { SQL } from "bun";
    ...
    if (raw.status !== 0)
        throw new Error(`placementContentInstanceId failed: ${raw.stderr}`);
    return raw.stdout;
}
test("AC #8: fork on /content/pages/<id> detaches placement; subsequent shared edit doesn't touch the forked page", async ({ page, }) => {
