...
function snapshotDrafts() {
    ...
    if (raw.status !== 0)
        throw new Error(`snapshotDrafts failed: ${raw.stderr || raw.stdout}`);
    return JSON.parse(raw.stdout.trim());
}
test.describe("e2e-livedit Genesis — divergent drafts + selection", () => {
    test.skip(process.env.CAELO_LIVEDIT_GENESIS !== "1", "opt-in (CAELO_LIVEDIT_GENESIS=1) — a Genesis turn spawns three parallel draft generations; nightly/on-demand cost, not per-PR");
    test("brief-rich prompt yields distinct drafts; operator selects one at /design/genesis", async ({ page, }) => {
        ...
        expect(drafts.length, "the site-genesis flow must save at least two candidate drafts").toBeGreaterThanOrEqual(2);
        const directions = new Set(drafts.map((d) => d.direction.trim().toLowerCase()));
        expect(directions.size, "draft directions must be distinct design angles, not repeats").toBe(drafts.length);
