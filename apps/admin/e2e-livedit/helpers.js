...
import { fetchVisionVerdict } from "./lib/vision-verdict.js";
 ...
 */
function runBunInline(script, extraEnv = {}) {
 ...
 */
export function resetLiveditFixtures() {
    runBunInline(`
    import { SQL } from "bun";
 ...
 */
export function seedMinimalSite() {
 ...
 */
export async function loginAsDevOwner(page) {
 ...
 */
export async function waitForChatTurnIdle(page, timeoutMs = 480_000) {
 ...
 */
export async function sendChatPromptAndWait(page, prompt, 
...
export function attachChatSessionTracker(page) {
    let current = null;
    page.on("request", (req) => {
        if (req.method() !== "POST")
            return;
 ...
 */
const DEFAULT_CONSOLE_ERROR_IGNORE = [
    {
...
export function attachBrowserConsoleErrorTracker(page, options = {}) {
    const errors = [];
    const customIgnore = options.ignore ?? (() => false);
    const shouldIgnore = (text) => {
        if (customIgnore(text))
            return true;
...
export function assertNoBrowserConsoleErrors(tracker) {
    const errors = tracker.drain();
    if (errors.length === 0)
        return;
    const quoted = errors.map((e) => `  - ${e.slice(0, 300)}`).join("\n");
    throw new Error(`assertNoBrowserConsoleErrors: ${errors.length} browser-side error(s) during this scenario. ` +
 ...
 */
export async function awaitStageComplete(page) {
    const responsePromise = page.waitForResponse((r) => r.url().includes("?/stageAndDeployStaging") && r.request().method() === "POST", 
    ...
    if (res.status() < 200 || res.status() >= 300) {
        throw new Error(`awaitStageComplete: stageAndDeployStaging returned HTTP ${res.status()}. Response: ${(await res.text()).slice(0, 500)}`);
    }
}
export async function awaitPublishComplete(page) {
    const responsePromise = page.waitForResponse((r) => r.url().includes("?/promoteToProduction") && r.request().method() === "POST", 
    // 360s since #155 — see waitForChatTurnIdle's rationale (hang guard,
    // not a race against legitimately longer turns/builds on a busy runner).
    { timeout: 360_000 });
    ...
    const res = await responsePromise;
    if (res.status() < 200 || res.status() >= 300) {
        throw new Error(`awaitPublishComplete: promoteToProduction returned HTTP ${res.status()}. Response: ${(await res.text()).slice(0, 500)}`);
    }
}
/**
 ...
 */
export function assertNoOrphanLocks(chatSessionId) {
    const out = runBunInline(`
    import { SQL } from "bun";
    const sql = new SQL(process.env.ADMIN_DATABASE_URL);
    await sql.begin(async (tx) => {
    ...
    if (n !== 0) {
        throw new Error(`assertNoOrphanLocks: chat_entity_locks has ${n} rows for chat session ${chatSessionId} after Stage/Publish (v0.10.19 regression class).`);
    }
}
/**
...
export function assertNoChatRunnerDiagWarnings() {
    if (!existsSync(ADMIN_LOG_PATH)) {
        throw new Error(`assertNoChatRunnerDiagWarnings: ${ADMIN_LOG_PATH} not found — global-setup did not capture admin stderr.`);
    }
    const log = readFileSync(ADMIN_LOG_PATH, "utf8");
    for (const pattern of DIAG_PATTERNS) {
        const m = log.match(pattern);
        if (m) {
            throw new Error(`assertNoChatRunnerDiagWarnings: regression-guard pattern hit. Matched: "${m[0].slice(0, 200)}". See ${ADMIN_LOG_PATH} for the full context.`);
        }
    }
}
/**
...
export function snapshotBackendLogOffset() {
    const startOffset = existsSync(ADMIN_LOG_PATH) ? statSync(ADMIN_LOG_PATH).size : 0;
    return { startOffset };
}
export function assertNoBackendErrors(tracker, options = {}) {
    if (!existsSync(ADMIN_LOG_PATH)) {
        throw new Error(`assertNoBackendErrors: ${ADMIN_LOG_PATH} not found — global-setup did not capture admin stderr.`);
    }
    ...
    for (const pattern of BACKEND_ERROR_PATTERNS) {
        ...
        if (!m)
            continue;
        if (ignore.some((re) => re.test(m[0])))
            continue;
        hits.push(m[0].slice(0, 400));
    }
    if (hits.length === 0)
        return;
    throw new Error(`assertNoBackendErrors: ${hits.length} backend error line(s) in admin.log (since byte ${tracker.startOffset}):\n` +
        hits.map((h) => `  - ${h}`).join("\n") +
 ...
 */
export async function verifyPublishedPageWithVision(page) {
    const apiKey = process.env.ANTHROPIC_API_KEY_E2E;
    if (!apiKey) {
        throw new Error("verifyPublishedPageWithVision: ANTHROPIC_API_KEY_E2E is unset (global-setup should have caught this).");
    }
