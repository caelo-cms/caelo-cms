...
test.describe("e2e-livedit — image generation (Nano Banana)", () => {
    test.skip(!process.env.GOOGLE_GENERATIVE_AI_API_KEY, "opt-in: needs GOOGLE_GENERATIVE_AI_API_KEY — real Nano Banana image gen (billed)");
    test.beforeEach(() => {
    ...
    test("AI generates a real image and it persists to media_assets", async ({ page }) => {
        ...
        await sendChatPromptAndWait(page, "Generate a brand-new AI image: a minimal, flat-design mountain logo in blue. " +
