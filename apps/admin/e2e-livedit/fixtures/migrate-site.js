...
const blogPost = (title, extra = "") => chrome(title, `<article><h1>${title}</h1><p>Aus der Werkstatt: ${title}. Hobeln, sägen, schleifen — ehrliche Arbeit.</p><p>Mehr davon im nächsten Beitrag.</p>${extra}</article><section><h2>Weitere Beiträge</h2><ul><li><a href="/blog/erster-beitrag.html">Erster Beitrag</a></li><li><a href="/blog/zweiter-beitrag.html">Zweiter Beitrag</a></li></ul></section>`);
const productPage = (name, price) => chrome(name, `<h1>${name}</h1><section class="card"><table><tr><td>Preis</td><td>${price}</td></tr><tr><td>Material</td><td>Esche & Stahl</td></tr></table></section><section class="card"><h2>Bestellen</h2><form action="/bestellen"><input name="menge" placeholder="Menge"><button>Anfragen</button></form></section>`);
export const MIGRATE_FIXTURE_ROUTES = {
    "/": {
        contentType: "text/html",
        body: chrome("Start", `<div class="hero"><h1>Handwerkzeug, das bleibt.</h1><p>Seit 1952 fertigen wir Hämmer, Sägen und Hobel in Freiburg.</p></div><section class="card"><h2>Unsere Klassiker</h2><ul><li><a href="/produkte/hammer.html">Schreinerhammer</a></li><li><a href="/produkte/saege.html">Gestellsäge</a></li></ul></section>`),
    },
    "/ueber-uns": {
        contentType: "text/html",
        // Seeded TYPO: "Addresse" (correct German: Adresse) — the
        // migration report must surface it.
        body: chrome("Über uns", `<h1>Über uns</h1><p>Drei Generationen Handwerk.</p><p>Unsere Addresse: Münsterplatz 1, 79098 Freiburg.</p>`),
    },
    "/kontakt": {
        contentType: "text/html",
        body: chrome("Kontakt", `<h1>Kontakt</h1><p>Schreiben Sie uns: werkstatt@steinmann.example</p>`),
    },
    "/blog/erster-beitrag.html": {
        contentType: "text/html",
        // Seeded DEAD LINK — /impressum-alt 404s.
        body: blogPost("Warum Eschenholz?", `<p>Rechtliches im <a href="/impressum-alt">alten Impressum</a>.</p>`),
    },
    ...
    "/versteckte-landingpage": {
        ...
        body: chrome("Aktion", `<h1>Frühjahrsaktion</h1><p>Nur über den Newsletter verlinkt — kein interner Link führt hierher.</p>`),
    },
    "/robots.txt": {
...
export function startMigrateFixtureSite() {
