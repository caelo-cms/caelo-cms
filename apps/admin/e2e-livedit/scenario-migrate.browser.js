...
async function waitForCrawlDone(timeoutMs) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        const rows = dbJson("return await tx`SELECT status FROM import_runs ORDER BY created_at DESC LIMIT 1`;");
...
test.describe("e2e-livedit migration — keep-design end to end", () => {
    test.skip(process.env.CAELO_LIVEDIT_MIGRATE !== "1", "opt-in (CAELO_LIVEDIT_MIGRATE=1) — crawl + multi-turn build is the epic's most expensive scenario");
    test("crawl → approve → per-type build with redirects and findings", async ({ page }) => {
        ...
        try {
            ...
            await sendChatPromptAndWait(page, `Meine bestehende Website ist ${site.url} — bitte übernehmt sie KOMPLETT in Caelo. Das Design soll erhalten bleiben. Ihr dürft direkt loslegen, sobald ich den Crawl freigegeben habe.`);
            const proposed = dbJson("return await tx`SELECT id::text AS id, status FROM import_runs ORDER BY created_at DESC LIMIT 1`;");
            ...
            expect(crawlStatus, "crawl must reach ready_for_review after the Approve click").toBe("ready_for_review");
            // #192 — the sitemap-only page was discovered.
            const hidden = dbJson("return await tx`SELECT count(*)::int AS n FROM import_pages WHERE source_url LIKE '%versteckte-landingpage%'`;");
            ...
            await page.goto("/edit");
            await sendChatPromptAndWait(page, "Der Crawl ist freigegeben und fertig. Bitte baut die Seite jetzt wie besprochen fertig — Seitentypen bestätigen wir so, wie ihr sie gruppiert habt.", 900_000);
            // Some builds take a second confirmation turn; nudge once if no
            // pages landed yet.
            const pagesAfterTurn2 = dbJson("return await tx`SELECT count(*)::int AS n FROM pages WHERE deleted_at IS NULL`;");
            if ((pagesAfterTurn2[0]?.n ?? 0) === 0) {
                await sendChatPromptAndWait(page, "Ja, die Gruppierung passt — bitte alle Seiten jetzt bauen.", 900_000);
            }
            // ── Structural floor ─────────────────────────────────────────
      ...
      `);
            ...
            expect(summary.templates, "clustering must yield MORE THAN ONE template (#194/#195 — the one-template collapse)").toBeGreaterThanOrEqual(2);
            expect(summary.redirects.length, "old .html URLs must 301 to the new paths (#196)").toBeGreaterThanOrEqual(1);
            ...
            let findingSurfaced = summary.notes > 0 ||
                /addresse|impressum-alt|toter link|dead link|schreibfehler|typo/i.test(transcript);
            if (!findingSurfaced) {
                await sendChatPromptAndWait(page, "Super — was ist euch beim Übernehmen inhaltlich aufgefallen? Bitte den Abschlussbericht (Tippfehler, tote Links, Verbesserungen).");
                transcript = await page.locator("ul").first().innerText();
                const notesAfter = dbJson("return await tx`SELECT count(*)::int AS n FROM import_pages WHERE notes IS NOT NULL`;");
                findingSurfaced =
                    (notesAfter[0]?.n ?? 0) > 0 ||
                        /addresse|impressum-alt|toter link|dead link|schreibfehler|typo/i.test(transcript);
            }
            expect(findingSurfaced, "the migration must surface the seeded typo or dead link (notes or closing report)").toBe(true);
        }
        finally {
            site.stop();
        }
