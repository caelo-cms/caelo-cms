 ...
 */
function writeMetricsArtifact() {
    if (!existsSync(SCENARIO_METRICS_JSONL))
        return;
    ...
    if (rows.length === 0 && sessionCount === 0)
        return;
    ...
    let totalCost = 0;
    for (const r of rows) {
        const n = (key) => typeof r[key] === "number" ? r[key] : Number.NaN;
        totalCost += Number.isFinite(n("costTotalUsd")) ? n("costTotalUsd") : 0;
        lines.push(`${String(r.scenario ?? "?").padEnd(15)} | ${String(n("loops")).padStart(5)} | ` +
    ...
    if (rows.length > 0) {
        ...
        lines.push(`${"TOTAL".padEnd(15)} | ${" ".repeat(5)} | ${" ".repeat(7)} | ${" ".repeat(7)} | ` +
