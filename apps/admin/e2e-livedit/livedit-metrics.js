...
function blocksFor(lines, marker) {
    const out = [];
 ...
 */
export function aggregate(loops, tools) {
    const turns = [];
...
export function estimateCostUsd(m) {
    const T = m.totals;
    const chat = (T.freshIn * RATES.freshInPerM +
...
export function formatReport(title, m) {
    const out = [`### ${title}`];
    for (const t of m.turns) {
        out.push(`\n-- turn ${t.turnNo} (${t.session.slice(0, 8)}) --`, "loop | stop        | srv | in     | read   | write  | fresh  | hit% | out    | tools");
        for (const r of t.rows) {
            out.push(`${String(r.loop).padStart(4)} | ${r.stop.padEnd(11)} | ${String(r.serverToolCalls).padStart(3)} | ` +
        ...
        const tt = t.totals;
        out.push(`  turn ${t.turnNo}: in=${k(tt.inCall)} read=${k(tt.cacheRead)} write=${k(tt.cacheWrite)} ` +
    ...
    out.push(`\nTOTALS: turns=${m.turns.length} loops=${T.loops} in=${k(T.inCall)} ` +
        ...
        `fresh=${k(T.freshIn)} (${m.freshPct}%) out=${k(T.out)}`, `COST (est): chat=${usd(c.chat)} + image=${usd(c.image)} (${c.imageCalls} img) = ${usd(c.total)}` +
...
export function checkThresholds(m, th) {
    const v = [];
...
export function logOffset(adminLogPath) {
    try {
 ...
 */
export function metricsBySession(logText) {
    ...
    for (const r of loops)
        bucket(r.session).loops.push(r);
    for (const t of tools)
        bucket(t.session).tools.push(t);
...
export function metricsSince(adminLogPath, offset) {
 ...
 */
export function recordScenarioMetrics(scenarioKey, m) {
