...
import { attachChatSessionTracker, loginAsDevOwner, resetLiveditFixtures, seedMinimalSite, sendChatPromptAndWait, } from "./helpers.js";
const PROMPT = "Add a footer with navigation links to Home, About, and Contact to every page of the site.";
...
function seedFooterBlock() {
    ...
    if (r.status !== 0)
        throw new Error(`seedFooterBlock failed: ${r.stderr || r.stdout}`);
}
/**
...
function primePostOnboarding() {
    ...
    if (r.status !== 0)
        throw new Error(`primePostOnboarding failed: ${r.stderr || r.stdout}`);
}
function snapshotFooter() {
    ...
    if (raw.status !== 0)
        throw new Error(`snapshotFooter failed: ${raw.stderr || raw.stdout}`);
    return JSON.parse(raw.stdout);
}
test("issue #106: AI adds a footer nav to the layout via add_module_to_layout (link-list, no punt)", async ({ page, }) => {
