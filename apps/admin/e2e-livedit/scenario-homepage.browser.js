...
import { assertNoChatRunnerDiagWarnings, assertNoOrphanLocks, attachChatSessionTracker, awaitPublishComplete, awaitStageComplete, getProductionUrl, loginAsDevOwner, resetLiveditFixtures, sendChatPromptAndWait, verifyPublishedPageWithVision, } from "./helpers.js";
...
const HOMEPAGE_PROMPT = "Create a homepage for an AI-first CMS called Caelo — a nicely designed page with a " +
...
// is unambiguous.
const HERO_REEDIT_PROMPT = "Update the hero headline to 'Ship faster with Caelo'. Keep all other modules unchanged.";
/**
 ...
 * seed pages.
 */
function snapshotMostRecentPage(sinceTimestamp) {
 ...
 * authored.
 */
function snapshotActiveThemeBrand() {
    const raw = spawnSync("bun", [
        "-e",
        `
        import { SQL } from "bun";
 ...
 * silently passing.
 */
function assertChromaticPrimary(value) {
    ...
    if (hex?.[1] !== undefined) {
        ...
        if (spread <= 24) {
            throw new Error(`active theme primary ${value} has RGB spread ${spread} (≤24 ≈ grayscale) — the AI shipped an off-brand neutral palette (issue #112)`);
        }
        return;
    }
    ...
    if (oklch?.[1] !== undefined) {
        ...
        if (chroma <= 0.03) {
            throw new Error(`active theme primary ${value} has OKLCh chroma ${chroma} (≤0.03 ≈ grayscale) — issue #112 regression`);
        }
        return;
    }
    throw new Error(`active theme primary ${value} is in an unrecognised color format — extend assertChromaticPrimary rather than skipping the on-brand check`);
}
test.describe("e2e-livedit Scenario 1 — homepage from scratch", () => {
    ...
    test("AI creates a homepage, stages, publishes, re-edits hero — vision verdict + regression guards", async ({ page, }) => {
        ...
        await expect(previewFrame.locator("h1").first(), "Expected the preview iframe to render at least one <h1>").toBeVisible({ timeout: 30_000 });
        await expect(previewFrame.locator("footer").first(), "Expected the preview iframe to render a <footer> element").toBeVisible({ timeout: 30_000 });
        ...
        await expect(previewFrame.locator("a:visible").first(), "Expected the rendered page to include at least one visible <a> (nav link, CTA, etc.)").toBeVisible({ timeout: 30_000 });
        // The rendered page must have non-trivial body text — guards
        // against an "empty page with layout chrome only" pass-through.
        const bodyText = (await previewFrame.locator("body").first().textContent({ timeout: 30_000 })) ?? "";
        expect(bodyText.trim().length, `Expected the rendered page body to contain substantive text. Got ${bodyText.trim().length} chars.`).toBeGreaterThan(100);
        // ── Step 4: Stage (AC #2, #7) ──────────────────────────────────
        ...
        expect(snapshot, "Expected the AI to create a page via add_page tool calls — no pages.updated_at > scenario start. Likely a v0.10.17-class empty-response regression.").not.toBeNull();
        if (!snapshot)
            throw new Error("unreachable");
        expect(snapshot.placements.length, `Expected ≥2 page_modules for ${snapshot.pageId}`).toBeGreaterThanOrEqual(2);
        ...
        const productionUrl = slugPath === "" || slugPath === "home"
            ? `${productionOrigin}/`
        ...
        expect(productionBody, "Published HTML must not leak {{# or {{/ markers — see #71, closes the open thread on #62").not.toMatch(/\{\{[#/]/);
        ...
        expect(themeBrand.origin, "Active theme origin must have flipped off 'seed' during the cold-start sequence").not.toBe("seed");
        expect((themeBrand.description ?? "").trim().length, "Active theme must carry a recorded design rationale (set_theme_meta / propose_create_theme description)").toBeGreaterThan(0);
        expect(themeBrand.primaryValue, "Active theme tokens must define color.primary (base token or 500 ramp stop)").not.toBeNull();
        if (themeBrand.primaryValue === null)
            throw new Error("unreachable");
        ...
        expect(design.chromaticColorCount, "Composed palette must carry at least two distinct chromatic colors (flat single-hue reads as unfinished — DEPTH_AND_SURFACE_HINTS)").toBeGreaterThanOrEqual(2);
        const designWarnings = [];
        if (!design.hasGradient)
            designWarnings.push("no gradient.* token composed");
        if (!design.hasSurfaceAlt)
            designWarnings.push("no color.surface-alt for section alternation");
        ...
        const changed = updatedAtPairs.filter((p) => p.postUpdatedAt !== null && (p.preUpdatedAt === null || p.postUpdatedAt > p.preUpdatedAt));
        expect(changed.length, `Expected ≥1 page_module_content row's updated_at to advance after the hero re-edit. updated_at pairs: ${JSON.stringify(updatedAtPairs)}`).toBeGreaterThanOrEqual(1);
        expect(changed.length, `Expected at least one placement to be untouched by the hero re-edit, but all ${updatedAtPairs.length} placements changed.`).toBeLessThan(updatedAtPairs.length);
        ...
        expect(violations, `homepage token/cache thresholds violated:\n${violations.map((x) => `  - ${x.message}`).join("\n")}`).toEqual([]);
    });
});
