...
const PROMPT = "Build a CTA-teaser module that has a nested Button module. " +
...
function snapshotNestedAuthoring() {
    ...
    if (raw.status !== 0)
        throw new Error(`snapshot failed: ${raw.stderr}`);
    return JSON.parse(raw.stdout);
}
test("AC #9: AI composes a CTA-teaser embedding a Button via the `module` field kind", async ({ page, }) => {
    ...
    const ctaValues = snap.ctaCi?.values;
