...
function loadDotEnvTestIfPresent() {
...
async function pollAdminReady() {
    ...
    throw new Error(`e2e-livedit: admin process did not become ready at ${ADMIN_BASE_URL}/login within ${ADMIN_READY_TIMEOUT_MS}ms (last error: ${lastError ?? "n/a"}). Check ${ADMIN_LOG_PATH}.`);
}
export default async function globalSetup() {
    loadEnvFile();
    loadDotEnvTestIfPresent();
    if (!process.env.ADMIN_DATABASE_URL) {
        throw new Error("e2e-livedit: ADMIN_DATABASE_URL must be set (compose stack up + .env loaded).");
    }
    const apiKey = process.env.ANTHROPIC_API_KEY_E2E;
    if (!apiKey || apiKey.trim().length === 0) {
        throw new Error("e2e-livedit: ANTHROPIC_API_KEY_E2E is not set.\n" +
    ...
    child.on("exit", (code, signal) => {
        if (code !== null && code !== 0) {
            ...
            console.error(`[e2e-livedit] admin process exited unexpectedly: code=${code} signal=${signal}. See ${ADMIN_LOG_PATH}.`);
        }
    });
