 ...
 *   }
 */
function parseChatRunnerLoops(log) {
    const re = /\[chat-runner\] loop \{\s*chatSessionId:\s*"([^"]+)",[\s\S]*?loopStop:\s*"([^"]+)",[\s\S]*?toolCalls:\s*(\d+),[\s\S]*?tokensIn:\s*(\d+),\s*tokensOut:\s*(\d+),?\s*\}/g;
    const out = [];
    let m;
    m = re.exec(log);
    while (m !== null) {
 ...
 */
function parsePlaywrightReport(json) {
    let parsed;
    try {
    ...
    function walk(node) {
        ...
        if (Array.isArray(o.suites)) {
            for (const child of o.suites)
                walk(child);
        }
    }
    walk(parsed);
    const totalDurationMs = parsed.stats?.duration ?? 0;
 ...
 */
export function parseAiCost(raw) {
    ...
    if (parsed === undefined) {
        ...
        if (first !== -1 && last > first)
            parsed = tryJson(trimmed.slice(first, last + 1));
    }
    ...
    if (!("calls" in o))
        return null;
    const byModel = Array.isArray(o.byModel)
        ? o.byModel.map((r) => {
            const row = (r ?? {});
            return {
 ...
 */
export function formatCostSection(cost) {
    if (!cost)
        return [];
    const lines = [];
    lines.push("### Real AI cost (this run)");
    lines.push("");
    lines.push(`- **${fmtUsd(cost.totalMicrocents)}** total across **${cost.calls}** call${cost.calls === 1 ? "" : "s"} (from \`ai_calls\`)`);
    lines.push(`- **${fmtTokens(cost.inputTokens)}** tokens in / **${fmtTokens(cost.outputTokens)}** out / **${fmtTokens(cost.cachedTokens)}** cached`);
    lines.push("");
    ...
    if (cost.unpricedCalls > 0) {
        lines.push(`**⚠️ ${cost.unpricedCalls} call(s) unpriced (missing \`ai_pricing\` row) — real cost is HIGHER than shown; add rates at \`/security/ai\`.**`);
        lines.push("");
    }
    return lines;
}
...
export function buildReport(inputs) {
    ...
    for (const l of loops)
        stopReasons.set(l.loopStop, (stopReasons.get(l.loopStop) ?? 0) + 1);
    const stopReasonStr = [...stopReasons.entries()]
        .sort((a, b) => b[1] - a[1])
        .map(([k, v]) => `${v}× \`${k}\``)
    ...
    if (turns === 0 && totalLoops === 0) {
        ...
        if (cost) {
            ...
            lines.push(`- **${fmtTokens(totalTokensIn)}** tokens in / **${fmtTokens(totalTokensOut)}** tokens out (loop-log, cumulative per turn)`);
        }
    ...
    if (results.length === 0) {
        ...
        lines.push("");
        lines.push(`**Total:** ${passed} passed, ${failed} failed in ${fmtMs(totalDurationMs)} wall-clock.`);
    }
    lines.push("");
    return lines.join("\n");
}
