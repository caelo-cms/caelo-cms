 ...
 *
 * UI-level pin on the same invariant the integration test enforces.
 * Uses the in-memory test-provider registry (P5.2 #1) so the two
 * contexts can register their own per-chat fixtures concurrently.
 */
...
test("two unpublished chats edit one module; main stays at original body", async ({ browser }) => {
  ...
  async function driveChat(ctxLabel: string, htmlEdit: string): Promise<void> {
    const providerName = `chat-iso-${ctxLabel}-${ts}`;
    await registerTestProvider(BASE, providerName, [
      [
        {
          kind: "tool-call",
          id: `tu-${ctxLabel}`,
    ...
    await attachTestProviderHeader(ctx, providerName);
    ...
    await ctx.close();
    await clearTestProvider(BASE, providerName);
  }

  await driveChat("A", "<p>edit-from-A</p>");
  await driveChat("B", "<p>edit-from-B</p>");

  const branchCount = runBunInlineCapture(
    `
    import { SQL } from "bun";
    const sql = new SQL(process.env.ADMIN_DATABASE_URL);
    await sql.begin(async (tx) => {
  ...
  expect(Number(branchCount)).toBeGreaterThanOrEqual(2);
});
