...
test.beforeAll(() => {
  ...
  runBunInline(
    ...
    await c.begin(async (tx) => {
      ...
      const m = ((await tx\`INSERT INTO modules (slug, display_name, type, html) VALUES (\${process.env.MOD_SLUG}, 'mv', \${process.env.MOD_SLUG}, '<p>MV</p>') RETURNING id::text AS id\`)[0])?.id;
      const pg = ((await tx\`INSERT INTO pages (slug, name, title, template_id, status) VALUES (\${process.env.PAGE_SLUG}, 'MV', 'MV', \${tpl}::uuid, 'draft') RETURNING id::text AS id\`)[0])?.id;
