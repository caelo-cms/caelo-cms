 ...
 */
export function runBunInline(script, extraEnv = {}) {
 ...
 * test.beforeAll.
 */
export function clearLoginRateBucket() {
    runBunInline(`
    import { SQL } from "bun";
 ...
 * the per-user policy.
 */
export function resetOverlayLayoutFor(email) {
    runBunInline(`
    import { SQL } from "bun";
    const sql = new SQL(process.env.ADMIN_DATABASE_URL);
    await sql.begin(async (tx) => {
 ...
 */
export async function registerTestProvider(baseURL, name, events) {
...
export async function clearTestProvider(baseURL, name) {
    await fetch(`${baseURL}/__test/providers?name=${encodeURIComponent(name)}`, {
        method: "DELETE",
    });
}
export async function attachTestProviderHeader(ctx, name) {
