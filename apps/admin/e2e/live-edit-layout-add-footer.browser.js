...
import { attachTestProviderHeader, clearLoginRateBucket, clearTestProvider, registerTestProvider, resetOverlayLayoutFor, runBunInline, } from "./helpers.js";
test.beforeAll(() => {
...
test("add_module (target=layout) reaches every page on the layout", async ({ context, page }) => {
    ...
    // Check both surfaces.
    expect(rows.some((row) => row.html.includes(FOOTER_TEXT) || (row.fields ?? "").includes(FOOTER_TEXT))).toBe(true);
});
