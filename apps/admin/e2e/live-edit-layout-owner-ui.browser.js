...
test("Owner creates a layout via /security/layouts/new then deletes it", async ({ page }) => {
    ...
    await page
        .getByLabel("HTML")
        .fill('<!doctype html><html><head><meta charset="utf-8"></head><body><header><caelo-slot name="header">_</caelo-slot></header><main><caelo-slot name="content">_</caelo-slot></main><footer><caelo-slot name="footer">_</caelo-slot></footer></body></html>');
