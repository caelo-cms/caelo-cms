...
const limit = Number.parseInt(args.find((a) => a.startsWith("--limit="))?.split("=")[1] ?? "20", 10);
...
async function main() {
    ...
    for (const t of targets)
        console.log(`  - ${t.original_name} (${t.id})`);
    if (dryRun) {
    ...
    for (const t of targets) {
        ...
        const r = await execute(registry, adapter, SYSTEM_CTX, "media.propose_alt", {
            ...
            rationale: "Auto-generated placeholder from filename. Replace with a description that explains what's visually informative for a screen reader.",
        });
        if (r.ok)
            written += 1;
    }
 ...
 */
function generateTemplateAlt(filename) {
    const base = filename.replace(/\.[a-z0-9]+$/i, "");
    const words = base.replace(/[-_]+/g, " ").trim();
    if (words.length === 0)
        return "Image (alt text needed)";
