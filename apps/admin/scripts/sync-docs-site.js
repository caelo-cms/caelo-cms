...
async function main() {
    ...
    for (const t of manifest.templates) {
        ...
        if (!blockSet.ok)
            console.error(`[docs-sync] template_blocks.set ${t.slug}:`, blockSet.error);
    }
    ...
    for (const p of manifest.pages) {
        ...
        for (const [index, block] of blocks.entries()) {
            ...
            if (moduleId) {
                ...
                if (!r.ok)
                    console.error(`[docs-sync] modules.update ${moduleSlug}:`, r.error);
            }
            else {
        ...
        if (moduleAttachments.length > 0) {
            ...
            if (!r.ok)
                console.error(`[docs-sync] pages.set_modules ${p.slug}:`, r.error);
        }
        ...
        if (frontmatter.seo?.description) {
            ...
            if (!seoResult.ok)
                console.warn(`[docs-sync] pages_seo.set ${p.slug}:`, seoResult.error);
        }
    }
