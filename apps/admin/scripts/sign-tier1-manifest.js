...
async function loadOrCreateDevKey(forceNew) {
    ...
    if (!forceNew && existsSync(DEV_KEY_PATH)) {
        const raw = JSON.parse(readFileSync(DEV_KEY_PATH, "utf8"));
        return raw;
    }
    ...
    for (const entry of readdirSync(PLUGINS_ROOT)) {
        ...
        if (!existsSync(distEntry)) {
            reports.push({
                slug: entry,
                status: "skipped",
                reason: "dist/index.js missing — run `bun run --filter ./packages/plugins/* typecheck` first",
            });
            continue;
        }
        try {
            ...
            reports.push({ slug: entry, status: "failed", reason: e.message });
        }
    }
    return reports;
}
