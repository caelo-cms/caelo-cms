...
Initialise the Bun monorepo (`apps/admin`, `apps/api-gateway`, `apps/static-generator`, `packages/shared`, `packages/query-api`, `packages/plugin-sdk`, `packages/provisioning`). Add TypeScript (strict), Biome, Zod, Vitest, a root `docker-compose.yml` with PostgreSQL, and a minimal GitHub Actions CI (typecheck + test). **Also write `CLAUDE.md` at the repo root** capturing the project-wide engineering principles any Claude session (or human contributor) should follow — see "CLAUDE.md contents" section below for the full spec. Deliverable: `bun install && docker compose up -d && bun test` passes on an empty scaffold; `CLAUDE.md` reviewed. No app logic.
