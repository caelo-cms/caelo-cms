...
- **svelte-check integration**: add `svelte-check` devDep to `@caelo/admin`; typecheck script becomes `svelte-kit sync && svelte-check --tsconfig tsconfig.json && tsc --noEmit`. Now `.svelte` scripts are type-checked too. CI already runs the admin typecheck.
...
- **P15.6 — Routing-manifest version negotiation.** `parseAndUpgradeManifest(raw, supportedVersions[])` in `@caelo/edge-router` so a manifest format change doesn't require a coordinated 3-stack redeploy. Edge handlers ship unchanged across version bumps as long as the parse helper covers the transition.
