...
**Phase 1 follow-up pass (P1.1)** — closes seven minor gaps from the post-P1 audit, all scoped to before P2:
- **Transaction rollback regression test.** New `rollback.integration.test.ts` asserts a handler that writes then throws leaves zero partial rows in `audit_events`.
- **NULL `actor_id` regression test.** Included in the rollback suite: INSERT with `NULL::uuid` fails (anchors the schema NOT NULL + RLS `NULLIF(...,'')` rejection).
- **`RateLimiter` stub interface.** `execute()` gains `ExecuteOptions.rateLimiter` + an `allowAllRateLimiter` default + a `RateLimited` variant in the `QueryError` union. No behaviour change in P1; P13 plugs in the gateway-backed limiter without touching the Query API surface.
- **Production `public_role` path covered by an integration test.** A second adapter instance constructed with `publicDatabaseUrl = PUBLIC_DATABASE_URL` (public_role) runs one RLS case end-to-end so the role actually used by the API Gateway in production is exercised, not just `admin_role` under FORCE RLS.
- **CI bootstrap invokes `packages/migrations/src/bootstrap.sh` directly** instead of duplicating role/DB creation SQL inline. Single source of truth; `PGHOST=localhost` + `PGPASSWORD` passed through so the same script runs for both initdb.d and the Ubuntu-runner path.
- **`__drizzle_migrations` gets RLS `ENABLE`** (not FORCE) plus an open policy, so the "every table has RLS" invariant is mechanical while the migration runner still operates as `admin_role`.
- **Plan rename.** The phase file says `bootstrap.sql`; actual file is `bootstrap.sh`. Update the stub to match the shell script (needed for env-var substitution at initdb time).

Verified locally + green remote CI; commit message pattern: `chore(phase-1): follow-up — rollback test, RateLimiter stub, public_role coverage, CI bootstrap.sh, meta-table RLS`.
...
**Phase 2 follow-up pass (P2.1)** — closes the ten audit gaps before P3 starts:
- **Security control panel stubs landed**: `/security` index + `/security/ai`, `/security/domains`, `/security/costs` as "Populated in P{N}" placeholders. `/security/roles` already existed from P2.
- **User management**: new Query API ops `users.list`, `users.create`, `users.set_roles`, `users.delete` + `/security/users` full CRUD UI. Unblocks the verification flow "Owner creates a custom role with a reduced permission set, assigns it to a user, and the user is blocked from out-of-scope routes". Create-user hashes password via Bun argon2id; role assignment is an atomic replace of `user_roles` rows inside one transaction.
- **End-to-end custom-role test**: a new `custom-role-enforcement.integration.test.ts` creates a custom role with a reduced permission set, creates a user, assigns the role, logs in as that user, and asserts `resolveSession` returns exactly the reduced permissions (no `deploy.trigger`, no `roles.manage`, etc.).
- **Reviewer-blocked test**: separate integration case that provisions a Reviewer user and asserts (a) resolved permissions exclude `deploy.trigger`; (b) a route calling `requirePermission(locals, 'deploy.trigger')` throws 403 for their session.
- **Audit logging invariant closed**: new `recordAudit(tx, op, actorId, succeeded, inputHash)` helper called from every op (`auth.login`, `auth.logout`, `users.*`, `roles.*`). `input_hash` is a sha256 of the canonical JSON of the input with sensitive fields (`password`) redacted before hashing. CLAUDE.md §7 *"all AI actions and DB ops pass through the audit log"* is no longer violated.
- **System actor seed**: new migration `0003_system_actor_seed.sql` inserts `(id='00000000-0000-0000-0000-00000000ffff', kind='system', display_name='Caelo System')` so the audit FK holds for pre-auth events. `hooks.server.ts`' `SYSTEM_CTX.actorId` references this exact UUID.
- **CSRF double-submit at the form layer**: `$lib/server/csrf.ts` exports `assertCsrfToken(form, locals)`. Every POST action (`/login`, `/logout`, `/setup`, `/security/users`, `/security/roles`) checks that the form's `_csrf` hidden input matches `locals.user?.csrfToken`. Authenticated form layouts inject the token via a shared `+layout.server.ts`; login + setup skip the check because the user has no session yet (Origin check + rate-limit cover those).
- **Login rate-limit wired**: per-IP sliding window (5 attempts / 5 minutes) using `$lib/server/rate-limit.ts`. Request's `x-forwarded-for` or `getClientAddress()`, keyed in an in-process `Map`. Exceeded → `Err('RateLimited')` returned through the P1.2 `ExecuteOptions.rateLimiter` hook. Auth logs still fire for denials.
- **svelte-check integration**: add `svelte-check` devDep to `@caelo-cms/admin`; typecheck script becomes `svelte-kit sync && svelte-check --tsconfig tsconfig.json && tsc --noEmit`. Now `.svelte` scripts are type-checked too. CI already runs the admin typecheck.
- **Setup race hardening**: `users.create_first_owner` takes `SELECT pg_advisory_xact_lock(736578)` ("setup" loosely) at the start of its transaction. Concurrent setup POSTs serialize; the loser sees the "setup already complete" branch correctly.
- **Playwright smoke**: `@playwright/test` devDep; `apps/admin/playwright.config.ts` with a `webServer` that runs `vite preview` after build; `apps/admin/e2e/setup-login.spec.ts` covers the full MVP flow (setup → login → dashboard → logout). CI gains a separate `e2e` job (after `check`) that installs the Playwright browser binary and runs the suite.

Dependencies added (all MPL-compatible): `svelte-check` 4.4.6 (MIT), `@playwright/test` 1.59.1 (Apache-2.0). No runtime-surface changes.
...
> **Superseded (epic #380, v0.12):** this phase shipped as designed and was then deliberately removed in the plugin-system-v2 cleanup (#381–#385) — core is locale-agnostic now. The capability returns as the first-party `international-site` plugin (CMS_REQUIREMENTS §7). The text below is kept as the historical build record; none of its surfaces exist in core anymore.
...
Provider adapters under the same Pulumi umbrella: GCP (Cloud SQL HA, Cloud Storage + CDN, Cloud Run, Secret Manager), AWS (RDS Multi-AZ, S3 + CloudFront, API Gateway + Lambda, Secrets Manager), Azure (Azure DB zone-redundant, Blob + CDN, API Management, Key Vault). Three-environment stacks per provider. Per-domain/subdomain SSL and mixed URL-shape routing handled by the adapter. Provider-specific redirect file generators (Cloudflare Pages `_redirects`, CloudFront Lambda@Edge, Azure Front Door) and per-provider CDN asset copy adapters. Admin DNS guidance page (§15.5) surfaces required DNS records per domain with resolution checks. **Per-provider edge A/B split adapters** — Cloud CDN / CloudFront (Lambda@Edge) / Azure Front Door rules implementing the same stable-hash split as P13's self-hosted gateway, with assignment logs piped into the analytics plugin. Patroni documented as opt-in HA for self-hosted.
...
## Master-plan audit follow-ups (gaps captured 2026-05-02; one-line owners only)

These are items the plan v1 missed that we found during the post-P15-PR3 audit. Each lands in the named phase as a sub-item — capture them here so they don't slip between phases:

- **P10A.1 — Skill effectiveness telemetry.** New `skill_engagements` table (skill_id, chat_session_id, fired_at, user_kept_engaged, conversation_concluded). `/security/skills` adds "Engagement rate (last 30d)", "Avg keep rate", "Last fired"; Owner can retire skills below thresholds. Without it, the `skills` table grows unbounded and dead skills inflate every system-prompt token bill.
- **P11.6 — Per-plugin AI cost cap + locale-aware declaration.** *(Part (b) superseded by epic #380 — the locale-column rule was deleted with the core i18n cut.)* (a) `plugins.ai_cost_cap_microcents` + per-plugin `ai_calls.plugin_id` rollup so a Tier-1 plugin's chat-runner-tool spawn can't drain the daily cap; admin `/security/plugins/[slug]` shows per-plugin spend. (b) Plugin manifest gains `localeAware: true|false` declared by the plugin author; validator ensures `locale` column when `true`, refuses `page_id` columns when `false`. Cross-locale data bleed becomes an AI-authored-plugin policy violation surfaced at validation, not a silent runtime drift.
- **P12.1 — Plugin SDK `purgeSubject(email|userId)` contract + GDPR right-to-be-forgotten.** Generic Plugin SDK requires every Tier-1/Tier-2 plugin that stores PII to implement `purgeSubject({email?, visitorId?})` returning row counts purged. Admin `/security/visitor-data` page calls every active plugin's purge for a given subject + writes audit. Without it Caelo is unusable for EU operators at OSS launch.
- **P13.1 — CDN cache invalidation step on deploy.** `cdn-copy.ts` adapters cover asset pinning but never invalidate HTML pages on CloudFront / Cloud CDN / Front Door after a static-gen deploy. New `CdnInvalidationAdapter.invalidate(paths[])` interface; per-provider implementations. `deploy.trigger` fires invalidation after copy. Without it every cloud deploy is "live in 60 s ... or 1 hour" depending on TTL.
- **P14.2 — Backup/restore drill** on a fresh VM, documented in `docs/operating-self-hosted.md`. Plus per-provider runbooks for cloud installs that translate "restore RDS PITR / Cloud SQL backup / Azure DB PITR" into one-command operator workflows (the existing `cms-provision restore` only speaks Compose). Without it the first real data-loss event is the first restore test.
- **P15.1 — `/api/internal/*` auth model.** Pick + document one scheme (signed JWT minted by Pulumi post-deploy with a short TTL is the lean choice). The `provisioning_outputs.set` op is system-only at the Query API but the HTTP boundary that fronts it still needs a contract. Sets the pattern for every future internal endpoint.
- **P15.2 — DNS auto-wire adapter.** New `@caelo-cms/dns-adapter` package with `Cloudflare`/`Route53`/`Cloud DNS`/`Azure DNS` impls behind one `DnsAdapter.upsertRecords(records[])` interface. Each cloud stack auto-detects same-provider hosting + writes records on `pulumi up` (operator provides registrar token via `pulumi config set --secret`). Reduces install-to-first-page from "30 min + DNS confusion" to "8 min + cert wait."
- **P15.3 — Cross-runtime byte-identity conformance harness.** `packages/edge-router/conformance/` boots the AWS L@E bundle (lambda-local), the GCP Cloud Run image, and the Caddy plugin (sidecar container) against a shared 1000-row corpus + asserts byte-identical assignments. Catches the silent-drift class of bug the in-process unit test can't.
- **P15.4 — Cloud provisioning E2E test gated `CAELO_RUN_CLOUD_E2E=1`.** `pulumi stack init test-<sha>` per provider, `pulumi up`, hit `/health`, assert 200 + valid cert + correct A/B assignment, `pulumi destroy --yes`. Nightly per provider on a single dedicated test project (~$50/mo). Without it, the first real cloud-stack bug surfaces at first operator install.
- **P15.5 — Stack-update drift banner on `/security/dns`.** `cms-provision pulumi-output-sync` tags the synced row with the Pulumi stack's update sequence number; admin shows "⚠ Stack updated since last sync" when the live sequence > the synced sequence. Plus an Owner-only "Refresh now" action that shells `pulumi-output-sync` server-side.
- **P15.6 — Routing-manifest version negotiation.** `parseAndUpgradeManifest(raw, supportedVersions[])` in `@caelo-cms/edge-router` so a manifest format change doesn't require a coordinated 3-stack redeploy. Edge handlers ship unchanged across version bumps as long as the parse helper covers the transition.
- **P16.1 — Structured logging + incident-response runbook.** Every Caelo service (admin, gateway, orchestrator, plugin-host, static-gen) emits structured JSON logs with `request_id` + `actor_id` + `op_name`. New `docs/incident-response.md` covers triage steps, log-correlation queries per provider, escalation pathways, postmortem template. P16 expands from "AI providers + cost dashboard" to "AI providers + cost dashboard + observability + ops runbook."
- **P16.2 — Telemetry / opt-out policy.** Document what Caelo itself phones home (default OFF + opt-in only). Cover: install pings, anonymized error reports, version-checks. MPL-2.0 community-trust hygiene. Pick the answer ONCE and document it before any default-on telemetry slips into a component.
- **P8.5 — Postgres full-text search foundation.** New `tsvector` columns on `pages`, `modules`, plugin tables that opt in. New `search.query` op + admin search box. AI's `## Pages` system-prompt block links to search instead of dumping every page id. Caelo's spec mentions content but no search phase; every site over ~50 pages hits a wall without it.
- **P17.0 — Release engineering pipeline.** New sub-phase before P17 docs. Covers: how is "Caelo v0.5.0" assembled, which Tier-1 manifests get signed for it, where signed bundles publish (`releases.caelo-cms.com/<version>/manifests.tar.gz`?), how does `bunx cms-provision` pull a *specific Caelo version* (`--version 0.5.0`), GitHub Actions release workflow, changelog generation. P17 docs ship for an unreleasable artifact without this.

**Other small audit findings already fixed in P15 hot-fixes:**
- Static-gen ↔ edge-router variant-path mismatch (every active A/B served 404 to non-control visitors). Fixed by adding `ab-routing.json` (edge-router shape) alongside the existing `routing-manifest.json` (deploy provenance) and pointing GCP/AWS edge handlers at the new filename.
...
- **i18n is plugin-owned (epic #380).** Core has no locale registry and no locale column on pages; the `international-site` plugin provides i18n (CMS_REQUIREMENTS §7). Locale/URL-strategy config goes through the §11.A proposal gate, never a direct AI write.
...
- **Missing translations = clean 404**, never a fallback. This is correct SEO behaviour — a requirement on the `international-site` plugin (CMS_REQUIREMENTS §7.2).
...
| P9 | *(removed — epic #380: core i18n deleted; tests deleted with #381–#385; capability returns as the `international-site` plugin)* | — | — |
| P10 | *(removed — epic #380: AI translation deleted from core; returns in the `international-site` plugin)* | — | — |
| **intl** *(epic #380 #394–#400, plugin-owned)* | slot alignment + full/update prompt builders + structural lock (`packages/plugins/international-site/src/translation.test.ts`); URL grammar + exclusive slots (`packages/plugin-host/src/url-composition.test.ts`) | scripted-provider translation flow incl. staleness + cap pause (`international-site-translation.integration.test.ts`); variant tools + gating markers (`international-site-tools.integration.test.ts`); hreflang/sitemap/selector (`international-site-hreflang.integration.test.ts`); shipped skills (`international-site-skills.integration.test.ts`); lifecycle incl. gated uninstall (`plugin-lifecycle.integration.test.ts`) | mock-AI: gated `set_locales` pauses on the in-chat card, write happens only after Approve, `/de/` variant + hreflang (`apps/admin/e2e/international-site.browser.ts`); real-AI: "übersetze die Seite ins Deutsche" end-to-end (`apps/admin/e2e-livedit/scenario-international-site.browser.ts`) |
...
| P11 | oxc-parser rule walker; Shadow-DOM component wrapper | sandbox subprocess boundary (fetch/fs/net blocked); activation gate rejects AI; plugin→cms_public migration | hello-world plugin end-to-end; Shadow-DOM CSS-leak test asserts host unaffected |
...
| P9 | *(removed — epic #380: the i18n Playwright flows were deleted with the core cut; the `international-site` plugin ships its own E2E rows)* |
| P10 | *(removed — epic #380: translation flows deleted with the core cut)* |
| **intl** | **Operator asks in German to translate a page: locale registry lands ONLY after the (auto-approved) gate, the `/de/` counterpart carries a localized slug + translated VALUES while the source page is untouched, published variants cross-link via hreflang incl. x-default — `e2e/international-site.browser.ts` (mock, incl. the pause-before-write proof) + `e2e-livedit/scenario-international-site.browser.ts` (real AI, §1A no-implementation-questions)** |
...
| P11 | Hello-world plugin: validator catches forbidden patterns; **human Owner activates** (AI cannot); **Shadow-DOM CSS leak test passes**; static render works |
